...
테스트 3 〉	통과 (0.16ms, 73.5MB)
테스트 4 〉	통과 (0.97ms, 78.5MB)
테스트 5 〉	통과 (0.13ms, 83MB)
테스트 6 〉	통과 (0.21ms, 72.3MB)
테스트 7 〉	통과 (0.20ms, 72.1MB)
테스트 8 〉	통과 (0.57ms, 72.3MB)
테스트 9 〉	통과 (0.55ms, 73.2MB)
테스트 10 〉	통과 (0.66ms, 72.5MB)
테스트 11 〉	통과 (0.59ms, 78MB)
채점 결과
정확성: 100.0
합계: 100.0 / 100.0
```
...
Stack을 이용해서 풀었다. move의 값을 이용해서 board에서 인형을 옮기고, 만약 옮기는 인형과 스택의 peek의 인형이 같다면 인형을 제거하고 answer의 값을 증가시켰다. 인형이 다르다면 그대로 뽑은 인형을 stack에 넣었다. 한번 돌때마다  board[j][move - 1] 을 0으로 만들어 주어 move을 조작 할 수 있도록 만들어 주었다.